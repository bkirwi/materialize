            ...
            while let Some(command) = command_rx.recv().await {
                match command {
                    AsyncStorageWorkerCommand::UpdateFrontiers(id, ingestion_description) => {
                        ...
                        let mut observed_since = Antichain::new();
                        ...
                        for (id, export) in ingestion_description.source_exports.iter() {
                            ...
                            // The as_of of the ingestion should normally be based
                            // on the upper frontiers of its outputs. However, when we encounter a
                            // new subsource for the first time,
                                ...
                                match seen_remap_shard.as_ref() {
                                    None => {
                                        let read_handle = client
                                            ...
                                            .await
                                            .unwrap();
                                        observed_since.clone_from(read_handle.since());
                        ...
                        for upper in resume_uppers.values() {
                            if let Some(upper) = upper.as_option() {
                                as_of.insert(upper.step_back().unwrap_or(T::minimum()));
                            }
                        }
                        if as_of.borrow() == AntichainRef::new(&[T::minimum()]) {
                            as_of = observed_since;
                        }
