...
impl StatsCursor {
    pub async fn new(
        ...
        let mk_filter = |name: &'static str, count: fn(&RelationPartStats) -> Option<usize>| {
        ...
        let errors = handle
            .snapshot_cursor(as_of.clone(), mk_filter("errors", |s| s.err_count()))
            .await?;
        let data = handle
            .snapshot_cursor(as_of.clone(), mk_filter("data", |s| s.ok_count()))
            .await?;
